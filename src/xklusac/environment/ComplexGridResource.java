 ...
 * <p>
 * The process of creating a Grid resource is as follows: <ol> <li> create PE
 * (Processing Element) objects with a suitable MIPS (Million Instructions Per
 * Second) or SPEC (Standard Performance Evaluation Corporation) rating; <li>
 * assemble them together to create a machine; <li> group one or more objects of
 * the machine to form a Grid resource </ol>
 * <p>
 ...
 * from this class <li> uses only a constructor from  {@link #ComplexGridResource(String, double, ResourceCharacteristics,
 *                              ResourceCalendar, AllocPolicy)} <li> overrides {@link #registerOtherEntity()}
 * method to register a different entity or tag to
 * {@link gridsim.GridInformationService}. However, you also need to create a
 * new child class extending from {@link gridsim.GridInformationService}. <li>
 * overrides {@link #processOtherEvent(Sim_event)} method to process other
 * incoming tags apart from the standard ones. </ol>
 ...
 * <li>A resource ID can be found using <tt>super.get_id()</tt> method from
 * {@link eduni.simjava.Sim_entity} class. </ul>
 * <p>
 * Since GridSim 3.1, a network framework has been incorporated into this
 * simulation. To make use of this, you need to create a resource entity only
 * using the below constructors: <ul> <li> {@link #ComplexGridResource(String, Link, ResourceCharacteristics,
...
public class ComplexGridResource extends GridResource {
    ...
    public void body() {
        ...
        super.send(super.output, GridSimTags.SCHEDULE_NOW, register,
                new IO_data(super.get_id(), SIZE, gisID));
    ...
    private void processEvent(Sim_event ev) {
        ...
        switch (ev.get_tag()) {
            ...
            case GridSimTags.RESOURCE_NUM_FREE_PE:
                ...
                super.send(super.output, 0.0, ev.get_tag(),
                        new IO_data(numFreePE, SIZE, src_id));
                break;
