...
public class AdvancedSpaceSharedWithRAM extends AllocPolicy {
    ...
    public void body() {
        ...
        while (Sim_system.running()) {
            ...
            if (ev.get_tag() == AleaSimTags.FAILURE_FINISHED) {
                ...
                super.sim_schedule(GridSim.getEntityId("Alea_Job_Scheduler"), 0.0, AleaSimTags.FAILURE_RESTART, this.resId_);
                continue;
            }
    ...
    public void gridletSubmit(Gridlet gl, boolean ack) {
        ...
        if (failure == false && success == true) {
            sim_schedule(GridSim.getEntityId("Alea_Job_Scheduler"), 0.0, AleaSimTags.GRIDLET_STARTED, gl);
        }
    ...
    private boolean allocatePEtoGridlet(ResGridlet rgl, int numPE) {
        ...
        MachineWithRAMandGPUs mach = (MachineWithRAMandGPUs) machines.get(ids[0]);
    ...
    private void gridletFinish(ResGridlet rgl, int status) {
        ...
        if (rgl.getNumPE() > 1) {
            ...
            MachineWithRAMandGPUs mr = null;
            for (int i = 0; i < machines.length; i++) {
                MachineWithRAMandGPUs mr_2 = (MachineWithRAMandGPUs) machinesList.getMachine(machines[i]);
                if (!mr_2.equals(mr)) {
                    mr = mr_2;
            ...
            MachineWithRAMandGPUs mr = (MachineWithRAMandGPUs) machinesList.getMachine(rgl.getMachineID());
        ...
        //super.sendFinishGridlet( rgl.getGridlet() );
        super.sim_schedule(GridSim.getEntityId("Alea_Job_Scheduler"), 0.0, GridSimTags.GRIDLET_RETURN, rgl.getGridlet());
     ...
     * numNodes;
     *
     * for (int i = 0; i < machines.size(); i++) { MachineWithRAMandGPUs machine =
     * (MachineWithRAMandGPUs) machines.get(i); // cannot use such machine if
