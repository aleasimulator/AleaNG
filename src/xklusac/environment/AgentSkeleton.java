...
public abstract class AgentSkeleton extends GridSim {
    ...
    /** Agent logic implementation. It reads incoming events and submits jobs to scheduler accordingly.
     *
    ...
    public final void body() {
        ...
        while (true) {
            ...
            // this agent still has jobs to process
            if (!finishing) {
                // if we do not have an enqueued job, get new job
                if (need_reset || waiting_job == null) {
                    waiting_job = this.getNextJob();
                    need_reset = false;
                }
     ...
     */
    abstract ComplexGridlet getNextJob();
