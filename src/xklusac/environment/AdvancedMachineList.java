 ...
 * @invariant $none
 */
public class AdvancedMachineList extends MachineList
{
    // 1. Create a separate index for fast lookups
    private Map<Integer, Machine> idIndex;

    public AdvancedMachineList() {
        super();
    ...
    public boolean add(Machine machine) {
        ...
        boolean added = super.add(machine);
        
        // If successful, add to our lookup index
        if (added) {
            idIndex.put(machine.getMachineID(), machine);
        }
        return added;
    }
