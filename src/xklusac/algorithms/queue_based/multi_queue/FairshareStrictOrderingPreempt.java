 ...
 */
public class FairshareStrictOrderingPreempt implements SchedulingPolicy {

    private Scheduler scheduler;

    public FairshareStrictOrderingPreempt(Scheduler scheduler) {
    ...
    public int selectJob() {
        ...
        for (int q = 0; q < Scheduler.all_queues.size(); q++) {
            ...
            for (int i = 0; i < Scheduler.active_scheduling_queue.size(); i++) {
                ...
                // try preemption (only non-preempted job with OK user/queue limits can evict other jobs)
                    ...
                    for (int j = 0; j < Scheduler.resourceInfoList.size(); j++) {
                        ...
                        if (Scheduler.isSuitable(ri, gi)) {
                            ...
                            if (checkpointed_jobs != null) {
                                ...
                                System.out.println(gi.getID() + ": Strict Ordering -> PREEMPT these " + printCheckpointedJobs(checkpointed_jobs) + " jobs. This job has status = " + gi.getGridlet().getGridletStatusString());
